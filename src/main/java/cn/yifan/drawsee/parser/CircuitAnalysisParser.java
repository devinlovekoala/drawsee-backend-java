...
/**
 * 电路分析专用响应解析器 针对电路分析结果提供更精确的节点提取
 *
 * @author yifan
...
public class CircuitAnalysisParser implements ResponseParser {
  ...
  /** 节段标记的正则表达式 匹配 [[SECTION:标题]] 格式的标记 */
  private static final Pattern SECTION_PATTERN =
      Pattern.compile("\\[\\[SECTION:(.*?)\\]\\]([\\s\\S]*?)(?=\\[\\[SECTION:|$)");

  /** 电路节点分析的正则表达式 特别用于提取Essential Nodes部分的信息 */
  private static final Pattern NODE_PATTERN =
      Pattern.compile("节点名称[：:](.*?)\\n.*?作用[：:](.*?)(?=\\n|$)", Pattern.DOTALL);

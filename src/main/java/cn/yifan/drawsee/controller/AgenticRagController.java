...
/**
 * Agentic RAG v2 控制器 提供Agentic RAG查询接口，对接前端Flow系统
 *
 * @author Drawsee Team
 * @date 2025-12-16
 */
...
public class AgenticRagController {
  ...
  @Autowired private AgenticRagService agenticRagService;

  /**
   * Agentic RAG 查询接口（SSE流式）
   *
   * <p>该接口对接Python Agentic RAG v2系统，提供： - 双层分类（输入类型 + 意图识别） - 智能路由到对应处理频道 - SSE流式返回结果
   *
   * @param query 用户查询
  ...
  public SseEmitter agenticQuery(
    ...
    log.info(
        "[AgenticRAG] 收到查询请求: userId={}, query='{}', kb_ids={}, has_image={}",
        userId,
  ...
  public SseEmitter agenticQueryPost(
      @RequestBody AgenticQueryRequest request, @RequestParam(required = false) String classId) {
    Long userId = StpUtil.getLoginIdAsLong();

    log.info(
        "[AgenticRAG] 收到POST查询请求: userId={}, query='{}', kb_count={}",
        userId,
  ...
  @SaCheckLogin
  public Map<String, Object> getChannelsStatus(@RequestParam(required = false) String classId) {
    Long userId = StpUtil.getLoginIdAsLong();
    ...
    if (status != null) {
      ...
      return Map.of("success", false, "error", "获取频道状态失败");
    }
  }

...
/**
 * @FileName AdminKnowledgeBaseController @Description 管理员知识库控制器类 @Author yifan
 *
...
public class AdminKnowledgeBaseController {
  ...
  public R<String> createKnowledgeBase(
    ...
    try {
      String knowledgeBaseId =
          knowledgeBaseService.createKnowledgeBaseByAdmin(createKnowledgeBaseDTO, isPublished);
      return R.ok(knowledgeBaseId);
    } catch (Exception e) {
      log.error("管理员创建知识库失败", e);
  ...
  public R<String> createRagKnowledgeBase(
      @RequestBody @Valid CreateKnowledgeBaseDTO createKnowledgeBaseDTO) {
    try {
      // 创建启用RAG的知识库
      String knowledgeBaseId =
          knowledgeBaseService.createRagEnabledKnowledgeBase(createKnowledgeBaseDTO);
      return R.ok(knowledgeBaseId);
    } catch (Exception e) {
  ...
  @SaCheckLogin
  @GetMapping("/rag/list")
  public R<List<RagKnowledgeVO>> getAdminRagKnowledges(
      @RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int size) {
    try {
      List<RagKnowledgeVO> ragKnowledgeBases =
          knowledgeBaseService.listAllRagKnowledgeBases(page, size);
  ...
  @SaCheckLogin
  @DeleteMapping("/{id}")
  public R<Boolean> deleteKnowledgeBase(@PathVariable("id") String id) {
    try {
  ...
  public R<String> addBilibiliResource(
      @RequestParam("url") String url, @ModelAttribute UploadResourceDTO uploadResourceDTO) {
    try {
      // 添加B站资源

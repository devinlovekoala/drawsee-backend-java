...
/**
 * @FileName StreamAiService @Description @Author yifan
 *
...
public class StreamAiService {
  ...
  @Autowired private StreamingChatModel qwenStreamingChatLanguageModel;
  @Autowired private StreamingChatModel deepseekV3StreamingChatLanguageModel;
  @Autowired private StreamingChatModel qwenVisionStreamingChatLanguageModel;
  ...
  /**
   * 根据指定角度生成详细回答 该方法由GeneralDetailWorkFlow调用，其中角度信息从父节点(ANSWER_POINT)中获取
  ...
  public void knowledgeDetailChat(
      List<ChatMessage> history,
  ...
  public void solverContinueChat(
      List<ChatMessage> history, String model, StreamingResponseHandler<AiMessage> handler) {
    LinkedList<ChatMessage> messages = new LinkedList<>(history);
  ...
  public void circuitAnalysisChat(
  ...
  /**
   * Tool-based chat with Agentic RAG integration 使用LangChain4j AiServices构建Assistant，支持Tool自动调用
   *
  ...
  public <T> void toolBasedChat(
    ...
    T assistant =
        AiServices.builder(assistantInterface)
            .streamingChatModel(chatModel)
            .tools(tools)
    ...
    try {
      ...
      tokenStream
          ...
          .onCompleteResponse(
              response -> {
                Response<AiMessage> wrapped =
                    Response.from(
                        response.aiMessage(), response.tokenUsage(), response.finishReason());
  ...
  private StreamingChatResponseHandler buildStreamingHandler(
      StreamingResponseHandler<AiMessage> handler) {
    return new StreamingChatResponseHandler() {
      ...
      public void onCompleteResponse(ChatResponse response) {
        Response<AiMessage> wrapped =
            Response.from(response.aiMessage(), response.tokenUsage(), response.finishReason());
        handler.onComplete(wrapped);
      }

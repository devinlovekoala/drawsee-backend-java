...
/**
 * @FileName PdfMultimodalService @Description PDF多模态分析服务，支持文本和图片联合分析 @Author yifan
 *
 * @date 2025-12-05
 */
@Service
...
public class PdfMultimodalService {
  ...
  @Autowired private ChatModel qwenVisionChatLanguageModel;

  @Autowired private MinioService minioService;

  @Autowired private PromptService promptService;

  @Autowired private CircuitImageNetlistParser circuitImageNetlistParser;
  ...
  public static class MultimodalAnalysis {
    ...
    public List<String> getImageAnalysis() {
      return imageAnalysis;
    }

    public void setImageAnalysis(List<String> imageAnalysis) {
      this.imageAnalysis = imageAnalysis;
    }

    public String getCombinedSummary() {
      return combinedSummary;
    }

    public void setCombinedSummary(String combinedSummary) {
      this.combinedSummary = combinedSummary;
    }

    public List<CircuitDesignResult> getCircuitDesigns() {
      return circuitDesigns;
    }

    public void setCircuitDesigns(List<CircuitDesignResult> circuitDesigns) {
      this.circuitDesigns = circuitDesigns;
    }
  }
  ...
  public static class CircuitDesignResult {
    ...
    public CircuitDesign getCircuitDesign() {
  ...
  public MultimodalAnalysis analyzePdfMultimodal(InputStream pdfInputStream, int maxImagePages) {
    ...
    try {
      ...
      // 2. 选择最复杂的页面进行图片分析
      List<Integer> topPages =
          PdfUtils.selectTopComplexPages(
      ...
      List<BufferedImage> images =
          PdfUtils.renderPages(new ByteArrayInputStream(pdfBytes), 150, topPages::contains);
      ...
      for (int i = 0; i < images.size(); i++) {
        ...
        if (imageAnalysis != null && !imageAnalysis.isBlank()) {
          result.getImageAnalysis().add(String.format("[第%d页图像分析]\n%s", pageNo + 1, imageAnalysis));
        }
        ...
        if (designResult != null) {
          ...
          log.info(
              "第{}页电路图成功转为结构化设计: elements={}, connections={}",
              pageNo + 1,
              designResult.getCircuitDesign().getElements() != null
                  ? designResult.getCircuitDesign().getElements().size()
                  : 0,
              designResult.getCircuitDesign().getConnections() != null
                  ? designResult.getCircuitDesign().getConnections().size()
                  : 0);
        }
      }
  ...
  private String analyzeImage(String base64Image, int pageNo) {
    try {
      ...
      UserMessage userMessage =
          UserMessage.from(
              TextContent.from(prompt), ImageContent.from("data:image/png;base64," + base64Image));

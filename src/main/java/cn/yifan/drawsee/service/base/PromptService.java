...
/**
 * @FileName PromptService @Description @Author yifan
 *
...
public interface PromptService {
  ...
  String getAnswerDetailPrompt(
      @PromptParam("question") String question, @PromptParam("angle") String angle);
  ...
  String getAnimationShotMergeCodePrompt(
      @PromptParam("shotInfoListString") String shotInfoListString);
  ...
  String getSolverFirstChatPrompt(
      @PromptParam("question") String question, @PromptParam("method") String method);
  ...
  /**
   * 获取PDF电路实验任务分析点提示词模板 用于解析PDF实验任务文档并生成分析点
   *
  ...
  /**
   * 获取PDF电路实验任务分析点详情提示词模板 用于展开特定分析点的详细内容
   *
   * @param text PDF文档提取的文本内容
   * @param angle 分析角度
   * @return 提示词内容
  ...
  String getPdfCircuitPointDetailPrompt(
      @PromptParam("text") String text, @PromptParam("angle") String angle);

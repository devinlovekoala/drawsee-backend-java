...
/**
 * @FileName AiService @Description @Author yifan
 *
...
public class AiService {
  ...
  @Autowired private ChatModel deepseekV3ChatLanguageModel;
  @Autowired private ChatModel qwenChatLanguageModel;
  ...
  public List<String> getRelatedKnowledgePoints(
      List<String> knowledgePoints, String question, AtomicLong tokens) {
    String prompt = promptService.getRelatedKnowledgePointsPrompt(knowledgePoints, question);
    Response<AiMessage> response =
        toResponse(deepseekV3ChatLanguageModel.chat(UserMessage.from(prompt)));
  ...
  public String recognizeTextFromImage(String imageUrl) {
    ...
    UserMessage userMessage =
        UserMessage.from(TextContent.from(imageTextPrompt), ImageContent.from(imageUrl));
    // 使用Qwen Vision模型进行识别
  ...
  public CircuitDesign recognizeCircuitDesignFromImage(String imageUrl) {
    ...
    UserMessage userMessage =
        UserMessage.from(TextContent.from(prompt), ImageContent.from(imageUrl));
    Response<AiMessage> response = toResponse(qwenVisionChatLanguageModel.chat(userMessage));
  ...
  public List<String> getPlannerSplit(LinkedList<ChatMessage> history, AtomicLong tokens)

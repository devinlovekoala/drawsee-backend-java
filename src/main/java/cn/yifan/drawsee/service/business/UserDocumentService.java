...
public class UserDocumentService {
  ...
  public UserDocument uploadDocument(
      MultipartFile file, Long userId, String title, String description) {
    ...
    try {
      ...
      String objectName =
          "user-documents/" + userId + "/" + System.currentTimeMillis() + "_" + fileName;
      ...
      // Some databases still mark file_url as NOT NULL, so persist object path to satisfy the
      // constraint.
    ...
    for (UserDocument doc : documents) {
      if (doc.getObjectPath() != null) {
        try {
          ...
          log.warn(
              "生成文档预签名URL失败: documentId={}, objectPath={}", doc.getId(), doc.getObjectPath(), e);
        }
      }
    }
    return documents;
  ...
  public UserDocument getDocumentById(Long id, Long userId) {
    ...
    if (document.getObjectPath() != null) {
      try {
        ...
        log.warn(
            "生成文档预签名URL失败: documentId={}, objectPath={}",
            document.getId(),
  ...
  public UserDocument getDocumentByUuid(String uuid, Long userId) {
    ...
    if (document.getObjectPath() != null) {
      try {
        ...
        log.warn(
            "生成文档预签名URL失败: documentUuid={}, objectPath={}",
            document.getUuid(),
            document.getObjectPath(),
            e);
      }

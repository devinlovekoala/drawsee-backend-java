...
public class RagEnhancementService {
  ...
  public RagChatResponseVO queryWithTimeout(
      ...
      RagQueryBudget ragBudget) {
    long timeoutMs =
        ragQueryProperties != null && ragQueryProperties.getRagTimeoutMs() != null
            ? ragQueryProperties.getRagTimeoutMs()
            : 1200L;
    try {
      CompletableFuture<RagChatResponseVO> future =
          CompletableFuture.supplyAsync(
              () ->
                  ragQueryService.queryWithBudget(
                      knowledgeBaseIds, prompt, null, userId, classId, ragBudget));
      return future.get(timeoutMs, TimeUnit.MILLISECONDS);
    } catch (TimeoutException e) {
      log.warn(
          "RAG检索超时，跳过增强: timeoutMs={}, prompt='{}...'",
          timeoutMs,

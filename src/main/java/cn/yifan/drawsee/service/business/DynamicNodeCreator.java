...
public class DynamicNodeCreator {
  ...
  public List<Node> createSubNodes(
      WorkContext workContext, String aiResponse, Node parentNode, String nodeType)
      throws JsonProcessingException {

    // 1. 选择合适的解析器
    ResponseParser parser = parserRegistry.getOrDefault(nodeType, parserRegistry.get("default"));
    ...
    for (NodeContent content : nodeContents) {
      Node node = createNodeFromContent(workContext, content, parentNode);
   ...
   * @throws JsonProcessingException JSON处理异常
   */
  private Node createNodeFromContent(WorkContext workContext, NodeContent content, Node parentNode)
      throws JsonProcessingException {

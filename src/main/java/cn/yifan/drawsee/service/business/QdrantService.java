...
public class QdrantService {
    ...
    public Map<String, Object> countPoints(String knowledgeBaseId, String documentId) {
        ...
        String url = baseUrl() + "/collections/" + properties.getCollection() + "/points/count?exact=true";
        Map<String, Object> body = new HashMap<>();
    ...
    public void upsertPoints(List<QdrantPoint> points) {
        ...
        log.info("Qdrant upsert: baseUrl={}, collection={}, points={}",
            baseUrl(), properties.getCollection(), points.size());
    ...
    private void put(String url, Object body) {
        ...
        HttpEntity<Object> entity = new HttpEntity<>(body, headers);
        try {
            Map response = restTemplate.exchange(url, org.springframework.http.HttpMethod.PUT, entity, Map.class).getBody();
            validateQdrantResponse(response);
        } catch (HttpClientErrorException ex) {
            String responseBody = ex.getResponseBodyAsString();
            throw new IllegalStateException("Qdrant 请求失败: " + responseBody, ex);

...
/**
 * @FileName KnowledgeBaseService @Description 知识库服务类 @Author yifan
 *
...
public class KnowledgeBaseService extends AbstractKnowledgeBaseService {
  ...
  @Autowired private EmbeddingModelProperties embeddingModelProperties;
  ...
  public String createKnowledgeBase(CreateKnowledgeBaseDTO createKnowledgeBaseDTO) {
    ...
    // 检查知识库名称是否已存在
    KnowledgeBase existKnowledgeBase =
        knowledgeBaseMapper.getByName(createKnowledgeBaseDTO.getName());
   ...
   */
  @Transactional(rollbackFor = Exception.class)
  public String createKnowledgeBaseByAdmin(
      CreateKnowledgeBaseDTO createKnowledgeBaseDTO, boolean isPublished) {
    // 检查知识库名称是否已存在
    KnowledgeBase existKnowledgeBase =
        knowledgeBaseMapper.getByName(createKnowledgeBaseDTO.getName());
    if (existKnowledgeBase != null) {
  ...
  public String joinKnowledgeBase(JoinKnowledgeBaseDTO joinKnowledgeBaseDTO) {
    ...
    KnowledgeBase knowledgeBase =
        knowledgeBaseMapper.getByInvitationCode(joinKnowledgeBaseDTO.getInvitationCode());
  ...
  public String addBilibiliResource(String url, UploadResourceDTO uploadResourceDTO) {
    ...
    // 验证知识库是否存在
    KnowledgeBase knowledgeBase =
        knowledgeBaseMapper.getById(uploadResourceDTO.getKnowledgeBaseId());
    if (knowledgeBase == null) {
  ...
  private List<RagKnowledgeVO> convertToRagVOList(List<KnowledgeBase> knowledgeBases) {
    ...
    return knowledgeBases.stream()
        .filter(
            kb ->
                Boolean.TRUE.equals(kb.getRagEnabled()) && !Boolean.TRUE.equals(kb.getIsDeleted()))
  ...
  private RagKnowledgeVO convertToRagVO(KnowledgeBase knowledgeBase) {
    int documentCount =
        knowledgeDocumentMapper.countCompletedByKnowledgeBaseId(knowledgeBase.getId());
    int chunkCount = knowledgeDocumentChunkMapper.countByKnowledgeBaseId(knowledgeBase.getId());

    String embeddingModel =
        embeddingModelProperties != null ? embeddingModelProperties.getModelName() : null;
  ...
  public List<KnowledgeBaseVO> getKnowledgeBasesForCurrentUser() {
    ...
    // 转换为VO列表
    return knowledgeBaseMap.values().stream().map(this::convertToVO).collect(Collectors.toList());
  }
   ...
   *
   * <p>权限规则： 1. 管理员创建的知识库 → 全局所有用户可访问 2. 教师创建的知识库 → 仅该教师开设班级中的学生可访问 3. 用户自己创建的知识库 → 用户自己可访问 4.
   * 用户作为成员的知识库 → 用户可访问
   *
   * @param userId 用户ID
  ...
  public List<String> getUserAccessibleKnowledgeBaseIds(Long userId, String classId) {
    ...
    try {
      ...
      if (allKbs != null) {
        for (KnowledgeBase kb : allKbs) {
          ...
          if (kb.getCreatorId() != null && userRoleService.isUserAdmin(kb.getCreatorId())) {
            ...
            log.debug(
                "[KnowledgeBase] 添加管理员知识库: kbId={}, creatorId={}", kb.getId(), kb.getCreatorId());
          }
        }
      }
      ...
      if (userCreatedKbs != null) {
        userCreatedKbs.stream().map(KnowledgeBase::getId).forEach(accessibleKbIds::add);
      }
      ...
      log.info(
          "[KnowledgeBase] 用户{}可访问的知识库: count={}, classId={}",
          userId,
          accessibleKbIds.size(),
          classId);

      return accessibleKbIds;

    } catch (Exception e) {
      log.error("[KnowledgeBase] 获取用户可访问知识库失败: userId={}, error={}", userId, e.getMessage(), e);
      // 失败时返回空列表，避免影响主流程
      return new ArrayList<>();
    }
  }

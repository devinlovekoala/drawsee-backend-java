...
/**
 * @FileName FlowService @Description @Author yifan
 *
...
public class FlowService {
  ...
  public List<AiTaskVO> getProcessingTasks(Long convId) {
    Long userId = StpUtil.getLoginIdAsLong();
    List<AiTask> aiTasks =
        aiTaskMapper.getByUserIdAndConvIdAndStatus(userId, convId, AiTaskStatus.PROCESSING);
    // 把task转换为taskVO，lambda玩法
    return aiTasks.stream()
        .map(
            (task -> {
  ...
  public CreateAiTaskVO createTask(CreateAiTaskDTO createAiTaskDTO) {
    ...
    if (createAiTaskDTO.getType().equals(AiTaskType.SOLVER_FIRST)
    ...
    if (createAiTaskDTO.getConvId() == null) {
      ...
      Node node =
          new Node(NodeType.ROOT, data, position, null, userId, conversation.getId(), false);
    ...
    AiTask aiTask =
        new AiTask(
            createAiTaskDTO.getType(),
            data,
            AiTaskStatus.WAITING,
            userId,
            createAiTaskDTO.getConvId());
  ...
  public void getCompletion(SseEmitter emitter, Long taskId) {
    ...
    RStream<String, Object> redisStream =
        redissonClient.getStream(RedisKey.AI_TASK_PREFIX + taskId);
    String errorMessage = getAiTaskError(aiTask, redisStream);

    if (errorMessage != null) {
      SseEmitter.SseEventBuilder event = SseEmitter.event().data(errorMessage).name("error");
      try {
        emitter.send(event);
      } catch (IOException e) {
    ...
    while (true) {
      ...
      Map<StreamMessageId, Map<String, Object>> messages =
          redisStream.read(
              StreamReadArgs.greaterThan(lastId)
                  .count(1)
                  .timeout(Duration.ofSeconds(0))); // TODO 可优化参数
      if (!messages.isEmpty()) {
        // 遍历获取Map中的每对消息
        for (Map.Entry<StreamMessageId, Map<String, Object>> entry : messages.entrySet()) {
          ...
          try {
            SseEmitter.SseEventBuilder event = SseEmitter.event().data(messageData).name("message");
            emitter.send(event);

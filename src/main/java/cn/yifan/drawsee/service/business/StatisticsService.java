...
public class StatisticsService {
    ...
    public StatisticsVO getStatistics() {
        ...
        try {
            // 异步获取用户统计
            CompletableFuture<StatisticsVO.UserStats> userStatsFuture = CompletableFuture.supplyAsync(this::getUserStats);
            
            // 异步获取会话统计
            CompletableFuture<StatisticsVO.ConversationStats> conversationStatsFuture = CompletableFuture.supplyAsync(this::getConversationStats);
            
            // 异步获取AI任务统计
            CompletableFuture<StatisticsVO.AiTaskStats> aiTaskStatsFuture = CompletableFuture.supplyAsync(this::getAiTaskStats);
            
            // 异步获取系统综合统计
            CompletableFuture<StatisticsVO.SystemStats> systemStatsFuture = CompletableFuture.supplyAsync(this::getSystemStats);
            
            // 等待所有异步任务完成并组装结果
            CompletableFuture.allOf(userStatsFuture, conversationStatsFuture, aiTaskStatsFuture, systemStatsFuture).join();
    ...
    private StatisticsVO.UserStats getUserStats() {
        ...
        // 获取每日新增用户趋势(最近30天)
        List<StatisticsVO.DailyStats> dailyNewUsers = convertToDailyStats(userMapper.countDailyNewUsers(30));
        
        // 获取每周新增用户趋势(最近12周)
        List<StatisticsVO.WeeklyStats> weeklyNewUsers = convertToWeeklyStats(userMapper.countWeeklyNewUsers(12));
        
        // 获取每月新增用户趋势(最近12个月)
        List<StatisticsVO.MonthlyStats> monthlyNewUsers = convertToMonthlyStats(userMapper.countMonthlyNewUsers(12));
    ...
    private StatisticsVO.ConversationStats getConversationStats() {
        ...
        // 获取每日新增会话趋势(最近30天)
        List<StatisticsVO.DailyStats> dailyNewConversations = convertToDailyStats(conversationMapper.countDailyNewConversations(30));
        
        // 获取每周新增会话趋势(最近12周)
        List<StatisticsVO.WeeklyStats> weeklyNewConversations = convertToWeeklyStats(conversationMapper.countWeeklyNewConversations(12));
        
        // 获取每月新增会话趋势(最近12个月)
        List<StatisticsVO.MonthlyStats> monthlyNewConversations = convertToMonthlyStats(conversationMapper.countMonthlyNewConversations(12));
    ...
    private StatisticsVO.AiTaskStats getAiTaskStats() {
        ...
        Map<String, Long> taskTypeDistribution = taskTypeDistributionData.stream()
        ...
        // 获取每日Token消耗趋势(最近30天)
        List<StatisticsVO.DailyStats> dailyTokenConsumption = convertToDailyStats(aiTaskMapper.countDailyTokenConsumption(30));
        
        // 获取每周Token消耗趋势(最近12周)
        List<StatisticsVO.WeeklyStats> weeklyTokenConsumption = convertToWeeklyStats(aiTaskMapper.countWeeklyTokenConsumption(12));
        
        // 获取每月Token消耗趋势(最近12个月)
        List<StatisticsVO.MonthlyStats> monthlyTokenConsumption = convertToMonthlyStats(aiTaskMapper.countMonthlyTokenConsumption(12));
    ...
    private StatisticsVO.SystemStats getSystemStats() {
        // 获取每日系统访问量趋势(最近30天)
        List<StatisticsVO.DailyStats> dailySystemVisits = convertToDailyStats(aiTaskMapper.countDailySystemVisits(30));
        
        // 获取每周系统访问量趋势(最近12周)
        List<StatisticsVO.WeeklyStats> weeklySystemVisits = convertToWeeklyStats(aiTaskMapper.countWeeklySystemVisits(12));
        
        // 获取每月系统访问量趋势(最近12个月)
        List<StatisticsVO.MonthlyStats> monthlySystemVisits = convertToMonthlyStats(aiTaskMapper.countMonthlySystemVisits(12));
        ...
        // 计算转化率(百分比)
        Double conversionRateRegToConv = registeredUsers > 0 ? (usersWithConversations * 100.0 / registeredUsers) : 0.0;
        Double conversionRateConvToAi = usersWithConversations > 0 ? (usersWithAiTasks * 100.0 / usersWithConversations) : 0.0;
        Double conversionRateRegToAi = registeredUsers > 0 ? (usersWithAiTasks * 100.0 / registeredUsers) : 0.0;
    ...
    private List<StatisticsVO.DailyStats> convertToDailyStats(List<Map<String, Object>> data) {
        return data.stream()
                .map(map -> StatisticsVO.DailyStats.builder()
    ...
    private List<StatisticsVO.WeeklyStats> convertToWeeklyStats(List<Map<String, Object>> data) {
        return data.stream()
                .map(map -> StatisticsVO.WeeklyStats.builder()
    ...
    private List<StatisticsVO.MonthlyStats> convertToMonthlyStats(List<Map<String, Object>> data) {
        return data.stream()
                .map(map -> StatisticsVO.MonthlyStats.builder()

...
public class ContextBudgetManager {
  ...
  public ContextBudgetPlan plan(String query, List<ChatMessage> history) {
    int maxContextTokens =
        getPositiveOrDefault(ragQueryProperties.getMaxContextTokens(), DEFAULT_MAX_CONTEXT_TOKENS);
    double safeRatio =
        getRatioOrDefault(ragQueryProperties.getSafeInputRatio(), DEFAULT_SAFE_INPUT_RATIO);
    int reservedOutputTokens =
        getPositiveOrDefault(
            ragQueryProperties.getReservedOutputTokens(), DEFAULT_RESERVED_OUTPUT_TOKENS);
    int safeInputTokens =
        Math.min(
            (int) Math.floor(maxContextTokens * safeRatio),
    ...
    int historyBudgetTokens =
        (int)
            Math.floor(
                safeInputTokens
                    * getRatioOrDefault(
                        ragQueryProperties.getHistoryRatio(), DEFAULT_HISTORY_RATIO));
    int retrievalBudgetTokens =
        (int)
            Math.floor(
                safeInputTokens
                    * getRatioOrDefault(
                        ragQueryProperties.getRetrievalRatio(), DEFAULT_RETRIEVAL_RATIO));
    ...
    int retrievalMaxTokens =
        Math.min(
            retrievalBudgetTokens, remainingTokens > 0 ? remainingTokens : retrievalBudgetTokens);

    int baseTopK =
        ragQueryProperties.getTopK() != null && ragQueryProperties.getTopK() > 0
    ...
    int historyRecentTurns =
        critical ? DEFAULT_CRITICAL_HISTORY_TURNS : DEFAULT_LIGHT_HISTORY_TURNS;
    int historySummaryTokens =
        critical ? DEFAULT_CRITICAL_SUMMARY_TOKENS : DEFAULT_LIGHT_SUMMARY_TOKENS;
  ...
  public HistoryCompressionResult compressHistoryIfNeeded(
      List<ChatMessage> history, ContextBudgetPlan plan) {

 ...
 *
 * 将Python的Agentic RAG系统包装为LangChain4j Tool
 * Java的ChatModel可以在需要时调用此Tool获取知识库检索结果
 *
 * @author Drawsee Team
 */
@Component
...
public class AgenticRagTool {
     ...
     *
     * 使用Agentic RAG系统进行智能路由和多频道处理：
     * - 自动识别输入类型（自然语言、公式、电路代码等）
     * - 自动分类意图（概念查询、计算、分析等）
     * - 路由到最合适的处理频道
     * - 返回高质量的检索或计算结果
     *
    ...
    public String searchKnowledgeBase(String query, List<String> knowledgeBaseIds) {
        try {
            log.info("[AgenticRagTool] 调用知识库检索: query='{}...', kb_count={}",
            ...
            if (Boolean.TRUE.equals(success)) {
                ...
                log.info("[AgenticRagTool] 检索成功: channel={}, answer_length={}",
                         channel, answer != null ? answer.length() : 0);
     ...
     * 简化版：仅传入查询文本，使用默认知识库
     */
    @Tool("Search knowledge base with default settings. Use when you need quick access to course materials.")

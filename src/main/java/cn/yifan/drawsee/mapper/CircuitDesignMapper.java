...
public interface CircuitDesignMapper {
     ...
     * @return 影响的行数
     */
    @Insert("INSERT INTO circuit_design (user_id, title, description, data, created_at, updated_at) " +
     ...
     * @return 影响的行数
     */
    @Update("UPDATE circuit_design SET title = #{title}, description = #{description}, " +
     ...
     * @return 电路设计数据
     */
    @Select("SELECT id, user_id, title, description, data, created_at, updated_at " +
     ...
     * @return 电路设计列表
     */
    @Select("SELECT id, title, description, created_at, updated_at " +
     ...
     * @return 影响的行数
     */
    @Update("UPDATE circuit_design SET is_deleted = 1, updated_at = NOW() " +

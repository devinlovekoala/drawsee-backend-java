...
public interface UserMapper {
    ...
    // 获取某个时间段内的新增用户数
    Long countNewUsersBetween(@Param("startTime") Timestamp startTime, @Param("endTime") Timestamp endTime);
    ...
    Long countActiveUsersBetween(@Param("startTime") Timestamp startTime, @Param("endTime") Timestamp endTime);
    
    // 获取留存率计算所需数据
    // 参数是基准日期，返回在该日期注册的用户中，在特定后续日期(次日/7天后/30天后)仍活跃的比例
    Double calculateRetentionRate(@Param("baseDate") Timestamp baseDate, @Param("afterDays") int afterDays);
}

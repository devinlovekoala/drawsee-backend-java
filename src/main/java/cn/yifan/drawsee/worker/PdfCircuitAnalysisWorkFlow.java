...
public class PdfCircuitAnalysisWorkFlow extends WorkFlow {
  ...
  /** PDF增强分析结果 */
  private record EnhancedAnalysisResult(
      String content, List<PdfMultimodalService.CircuitDesignResult> circuitDesigns) {}
  ...
  /** 覆盖父类方法，为PDF电路实验任务分析流程提供优化的实现 不创建父角度节点，而是仅保留QUERY节点作为后续分析点节点的父节点 */
  @Override
  ...
  /** 设置一个虚拟的流式节点，用于后续的分析点节点创建 该节点不会实际插入数据库，仅用于后续处理 */
  private void setupVirtualStreamNode(WorkContext workContext, Long parentNodeId)
    ...
    workContext
        ...
        .add(
            StreamAddArgs.entries(
                "type", cn.yifan.drawsee.constant.AiTaskMessageType.TITLE, "data", title));
  }
  ...
  public void streamChat(WorkContext workContext, StreamingResponseHandler<AiMessage> handler)
    ...
    log.info("PDF电路分析工作流生成提示词完成，内容长度: {}, taskId: {}", prompt.length(), aiTaskMessage.getTaskId());
    ...
    if (aiTaskMessage.getClassId() != null && !aiTaskMessage.getClassId().isBlank()) {
      knowledgeBaseIds =
          classKnowledgeService.getAccessibleKnowledgeBaseIds(
              Long.parseLong(aiTaskMessage.getClassId()), aiTaskMessage.getUserId());
    } else {
      knowledgeBaseIds =
          classKnowledgeService.getAccessibleKnowledgeBaseIds(null, aiTaskMessage.getUserId());
    }
    ...
    log.info(
        "[PdfCircuitAnalysis-Tool] 开始Tool-based对话生成(无外部RAG), taskId={}", aiTaskMessage.getTaskId());
   ...
   *
   * <p>注意：用户上传的PDF实验任务文档使用传统多模态分析方案，不调用Python RAG服务 Python RAG服务仅用于检索后台导入的知识文档
   *
    ...
    try {
      ...
      if (analysis.getCircuitDesigns() != null && !analysis.getCircuitDesigns().isEmpty()) {
        ...
        for (PdfMultimodalService.CircuitDesignResult designResult : analysis.getCircuitDesigns()) {
          ...
          int connectionCount =
              design.getConnections() != null ? design.getConnections().size() : 0;
          enhancedContent.append(
              String.format(
                  "第%d页电路图已还原为可视化设计，元件数: %d，连线数: %d。\n",
  ...
  /** 从 MinIO 预签名 URL 解析 objectName 并抽取 PDF 文本 返回抽取的文本；若失败返回 null */
   ...
   * 从知识库检索相关电路原理知识
   *
   * <p>Note: Tool-based架构下，此方法已废弃 RAG查询现在通过AgenticRagTool在LLM调用时自动完成
   *
  ...
  private String extractCircuitQuery(String pdfContent) {
    ...
    // 提取前500个字符作为查询上下文
    String queryContext = pdfContent.length() > 500 ? pdfContent.substring(0, 500) : pdfContent;
  ...
  private void processPdfCircuitAnalysisPoints(
      WorkContext workContext, String responseText, Long parentId) throws JsonProcessingException {
    AiTaskMessage aiTaskMessage = workContext.getAiTaskMessage();
    ...
    for (String s : lines) {
      ...
      if (line.isEmpty()) {
        ...
        if (currentTitle != null && currentDescription != null) {
          createPdfCircuitPointNode(
              workContext, parentId, aiTaskMessage, currentTitle, currentDescription);
          created++;
          currentTitle = null;
    ...
    if (created == 0 && responseText != null && !responseText.isBlank()) {
      ...
      String fallbackDesc =
          responseText.length() > 1200 ? responseText.substring(0, 1200) + "..." : responseText;
  ...
  /** 创建PDF电路实验任务分析点节点 */
  private void createPdfCircuitPointNode(

...
/**
 * @FileName KnowledgeDetailWorkFlow @Description 知识点详情AI工作流 - MySQL版本 @Author yifan
 *
...
public class KnowledgeDetailWorkFlow extends WorkFlow {
  ...
  public void streamChat(WorkContext workContext, StreamingResponseHandler<AiMessage> handler)
    ...
    Map<String, Object> parentNodeData =
        objectMapper.readValue(parentNode.getData(), parentNodeDataTypeReference);
  ...
  public void createOtherNodesOrUpdateNodeData(WorkContext workContext)
    ...
    List<KnowledgeResource> resources =
        knowledgeResourceMapper.getByKnowledgeId(knowledge.getId(), false);
   ...
   * @return 知识点实体
   */
  private Knowledge findKnowledgeInUserBases(
      Long userId, String knowledgePointName, AiTaskMessage aiTaskMessage) {
    // 获取当前任务的班级ID
    String classId = aiTaskMessage.getClassId();
    ...
    if (classId != null && !classId.isEmpty()) {
      ...
      log.info(
          "在指定班级中未找到知识点，将查找用户其他班级: classId={}, knowledgePoint={}", classId, knowledgePointName);
    }
    ...
    for (ClassMember classMember : userClassMembers) {
      ...
      for (String knowledgeBaseId : knowledgeBaseIds) {
        ...
        if (!knowledgeBase.getIsPublished() && !knowledgeBase.getCreatorId().equals(userId)) {
          log.info(
              "知识库未发布且用户非创建者，跳过: knowledgeBaseId={}, creatorId={}",
              knowledgeBase.getId(),
              knowledgeBase.getCreatorId());
          continue;
        }

        // 在知识库中查找指定的知识点
        List<Knowledge> knowledgeList =
            knowledgeMapper.getByKnowledgeBaseId(knowledgeBaseId, false);
        for (Knowledge foundKnowledge : knowledgeList) {
          if (foundKnowledge.getName().equals(knowledgePointName)) {
            log.info(
                "在知识库中找到知识点: knowledgeBase={}, knowledgePoint={}",
    ...
    for (KnowledgeBase knowledgeBase : userKnowledgeBases) {
      ...
      List<Knowledge> knowledgeList =
          knowledgeMapper.getByKnowledgeBaseId(knowledgeBase.getId(), false);
      for (Knowledge foundKnowledge : knowledgeList) {
        if (foundKnowledge.getName().equals(knowledgePointName)) {
          log.info(
              "在用户直接加入的知识库中找到知识点: knowledgeBase={}, knowledgePoint={}",
              knowledgeBase.getName(),
              knowledgePointName);
    ...
    for (String knowledgeBaseId : knowledgeBaseIds) {
      ...
      for (Knowledge knowledge : knowledgeList) {
        if (knowledge.getName().equals(knowledgePointName)) {
          log.info(
              "在班级相关知识库中找到知识点: classId={}, knowledgeBase={}, knowledgePoint={}",
              classId,
              knowledgeBase.getName(),
              knowledgePointName);
          return knowledge;
  ...
  private Knowledge findKnowledgeInPublishedBases(String knowledgePointName) {
    ...
    for (KnowledgeBase knowledgeBase : publishedBases) {
      ...
      for (Knowledge knowledge : knowledgeList) {
        if (knowledge.getName().equals(knowledgePointName)) {
          log.info(
              "在公开知识库中找到知识点: knowledgeBase={}, knowledgePoint={}",
              knowledgeBase.getName(),
              knowledgePointName);
          return knowledge;
        }

...
public class PdfCircuitAnalysisDetailWorkFlow extends WorkFlow {
  ...
  /** 覆盖父类方法，避免创建QUERY节点 直接使用PDF_CIRCUIT_POINT节点作为详情回答的父节点 */
  @Override
  ...
  public void streamChat(WorkContext workContext, StreamingResponseHandler<AiMessage> handler)
    ...
    if (extractedText != null && !extractedText.isBlank()) {
      ...
      log.info(
          "PDF电路分析详情工作流：使用抽取的PDF正文生成提示词，chars={}, angle={}", extractedText.length(), angleTitle);
    } else {
      // 回退：直接把 URL 作为文本放入模板
      prompt =
          promptService.getPdfCircuitPointDetailPrompt(
              "文档链接：" + pdfUrl + "\n(如无法访问链接，请根据常见电路实验任务要点给出通用分析)", angleTitle);
      log.info("PDF电路分析详情工作流：未能抽取正文，回退为URL兜底");
    }

    LinkedList<ChatMessage> history =
        applyHistoryBudget(workContext, planContextBudget(workContext, prompt));

    // 构建Agentic RAG查询（PDF分析点详情）
    String detailQuery =
        String.format(
            "根据实验任务文档，详细分析【%s】这个角度：\n\n%s",
            angleTitle, extractedText != null ? extractedText : "文档链接：" + pdfUrl);

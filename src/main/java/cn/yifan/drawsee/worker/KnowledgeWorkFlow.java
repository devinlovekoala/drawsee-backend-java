...
/**
 * @FileName KnowledgeWorkFlow @Description 知识点分点AI工作流 - MySQL版本 @Author yifan
 *
...
public class KnowledgeWorkFlow extends WorkFlow {
  ...
  public void createOtherNodesOrUpdateNodeData(WorkContext workContext)
    ...
    if (isCircuitGenerationIntent(aiTaskMessage.getPrompt())) {
      try {
        ...
        log.warn(
            "知识流文本生成电路失败，回退到知识点分支: taskId={}, reason={}",
    ...
    List<String> relatedKnowledgePoints =
        aiService.getRelatedKnowledgePoints(
            knowledgePoints, aiTaskMessage.getPrompt(), workContext.getTokens());
  ...
  private void createGeneratedCircuitNodes(
      WorkContext workContext, Node streamNode, AiTaskMessage aiTaskMessage)
      throws JsonProcessingException {
    if (streamNode == null) {
      return;
    }
   ...
   */
  private void addKnowledgePointsFromBase(
      KnowledgeBase knowledgeBase, Set<String> knowledgePoints) {
    try {
      // 从知识库中获取所有知识点
  ...
  public void streamChat(
      ...
      dev.langchain4j.model.StreamingResponseHandler<dev.langchain4j.data.message.AiMessage>
          handler)
      throws com.fasterxml.jackson.core.JsonProcessingException {
    AiTaskMessage aiTaskMessage = workContext.getAiTaskMessage();
    java.util.LinkedList<dev.langchain4j.data.message.ChatMessage> history =
        workContext.getHistory();
    ...
    try {
      ...
      if (!knowledgeBaseIds.isEmpty()) {
        ...
        var ragResponse =
            ragEnhancementService.queryWithTimeout(
                knowledgeBaseIds, originalPrompt, history, userId, String.valueOf(classId));
        ...
        if (ragResponse != null
          ...
          log.info(
              "RAG Prompt增强成功: 检索到{}个相关电路图, 增强后Prompt长度={}",
              ragResponse.getChunks() != null ? ragResponse.getChunks().size() : 0,

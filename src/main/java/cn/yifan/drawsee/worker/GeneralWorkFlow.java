...
/**
 * @FileName GeneralWorkFlow @Description 处理通用对话任务的工作流 @Author yifan
 *
...
public class GeneralWorkFlow extends WorkFlow {
  ...
  /** 覆盖父类方法，为通用问答流程提供优化的实现 不创建父角度节点，而是仅保留QUERY节点作为后续角度节点的父节点 */
  @Override
  ...
  /** 设置一个虚拟的流式节点，用于后续的回答角度节点创建 该节点不会实际插入数据库，仅用于后续处理 */
  ...
  /** 覆盖父类方法，为通用问答流程提供优化的实现 不创建流式节点，直接使用查询节点作为父节点 */
  @Override
  ...
  private RagChatResponseVO tryRagEnhance(WorkContext workContext, List<ChatMessage> history) {
    ...
    return ragEnhancementService.queryWithTimeout(
        knowledgeBaseIds, aiTaskMessage.getPrompt(), history, userId, String.valueOf(classId));
  }

  /** 覆盖父类方法，修改createAnswerPointNode的行为 使角度节点直接挂在QUERY节点下 */
  @Override
  protected void createAnswerPointNodes(WorkContext workContext) throws JsonProcessingException {
    ...
    dev.langchain4j.model.output.Response<AiMessage> streamResponse =
        workContext.getStreamResponse();
    AiTaskMessage aiTaskMessage = workContext.getAiTaskMessage();
  ...
  private void processAnswerAngles(WorkContext workContext, String responseText, Long parentId)
      throws JsonProcessingException {
    AiTaskMessage aiTaskMessage = workContext.getAiTaskMessage();
  ...
  /** 创建答案角度节点，直接挂在查询节点下 */
  private void createAnswerPointNode(
  ...
  private void createGeneratedCircuitNodes(
      WorkContext workContext, Long parentId, AiTaskMessage aiTaskMessage)
      throws JsonProcessingException {

-- Fast table-only alignment: drop extra tables, create missing tables.
-- Does NOT modify columns inside existing tables.
...
-- Drop tables not in expected list
SET @drop_tables_sql = (
  SELECT IFNULL(
    GROUP_CONCAT(CONCAT('DROP TABLE IF EXISTS `', t.table_name, '`') SEPARATOR '; '),
    'SELECT 1'
  )
...
-- Create missing tables (no column-level changes for existing tables)
...
CREATE TABLE IF NOT EXISTS `rag_ingestion_task` (
  ...
  `knowledge_base_id` varchar(255) NOT NULL COMMENT '知识库ID',
  `document_id` varchar(64) NOT NULL COMMENT '文档ID',
  `stage` varchar(50) DEFAULT NULL COMMENT '处理阶段',
...
CREATE TABLE IF NOT EXISTS `circuit_design` (
  ...
  `title` varchar(255) NOT NULL COMMENT '标题',
  `description` text DEFAULT NULL COMMENT '描述',
  `data` longtext DEFAULT NULL COMMENT '电路JSON数据',
  `created_at` datetime NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `is_deleted` tinyint(1) NULL DEFAULT 0 COMMENT '逻辑删除标记',
  PRIMARY KEY (`id`) USING BTREE,
